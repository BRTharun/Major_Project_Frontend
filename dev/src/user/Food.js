          ...
          <div key={index} className="recipe-card" style={recipeCardStyle}>
            {recipeResult[0] ? (
              <>
                ...
                <h4 style={recipeIngredientsTitleStyle}>Ingredients:</h4>
                <ul style={recipeIngredientsListStyle}>
                  {recipeResult[0].recipe.ingredients.map((ingredient, ingredientIndex) => (
                    <li key={ingredientIndex} style={recipeIngredientStyle}>{ingredient.text}</li>
                  ))}
                </ul>
                <a href={recipeResult[0].recipe.url} target="_blank" rel="noopener noreferrer" style={recipeLinkStyle}>
                  View Recipe
                </a>
              </>
            ) : (
              <p style={recipeNoDataStyle}>No recipe data available</p>
            )}
          </div>
        ))}
      </div>
